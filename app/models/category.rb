class Category
  ...
  def child_cat_new # рекурсивная функция
    str = ''
    if self.children.any? 
      ...
      self.children.each do |child|
        if child.children.any? # если есть подкатегории, то добавить стрелочку
          str += "<div class='category_new' data-category=#{child.name.gsub(' ', '_')}>#{child.name}<i class='fa fa-caret-right'></i></div>"
        else # иначе просто названеие
          str += "<div class='category_new' data-category=#{child.name.gsub(' ', '_')}>#{child.name}</div>"
        end
      end
      str += "</div>"
      self.children.each do |child|
        str += child.child_cat_new
      end
    end    
    str.html_safe
  end
